...
program.version(packageInfo.version).option('create [email] [password]', 'create session: email & password is optional and can be supplied in runtime.').option('delete', 'delete session: delete the stored credentials if they exist.').option('check', 'check session: check if session is alive').parse(process.argv);
