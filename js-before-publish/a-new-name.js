 ...
 *
 *  This event handler runs on after presence event. 
 *
...
function process(request) {
    ...
    /*
     *  Since this is a after presence event hanlder, it gets invoked when
     *  a uuid joins a channel, we can take any action which would make sense
     *  on arrival of a new uuid in channel. One of the probable action could
     *  be keeping a log of user arrival and departure activity in chatroom.
     *  After saving, we'll publish this object to response channel, and we
     *  can verify in our test case, if the activity was recorded correctly
     *
     */
    ...
    try {
        ...
        /*
         * for our demo example, we will just keep a record of join and leave
         * events for uuids in key value store provided by blocks
         * Sample request
        ...
        var record_for_uuid = [];
        if (request.message.uuid in activity_record) {
          record_for_uuid = activity_record[request.message.uuid]
        }
