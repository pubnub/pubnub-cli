...
describe('#components/session', () => {
  ...
  describe('#_createSessionFile', () => {
    it('returns the contents of the session file', (done) => {
      ...
      sessionComponentInstance._createSessionFile('nice').then(() => {
        ...
        assert.equal(fs.writeFile.args[0][1], 'nice');
        done();
      });
    });
  });
  ...
  describe('#_fetchSessionFile', () => {
    it('returns the contents of the session file', (done) => {
      sandbox.stub(fs, 'readFile', (path, encdoing, callback) => { callback(null, 'hello'); });
      sessionComponentInstance._fetchSessionFile().then((sessionId) => {
        assert.equal(sessionId, 'hello');
